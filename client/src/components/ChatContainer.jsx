    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>
            ...
            {onlineUser.includes(selectedUser._id) && <span className='w-2 h-2 
      ...
      {/* bottom area */}
      <div className=' h-15 absolute bottom-0 left-0 right-0 flex items-centre gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
            <input onChange={(e)=> setInput(e.target.value)} 
            value={input} onKeyDown={(e)=>e.key==="Enter" ? handleSendMessage(e) : null} 
            type="text" placeholder='Send a message' 
            className='flex-1 test-sm px-3 rounded-lg border-none outline-none 
