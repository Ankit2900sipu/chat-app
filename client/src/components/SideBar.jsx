    ...
    text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      ...
      <div className='flex flex-col'>
        {filteredUser.map((user,index)=>(
            ...
            }} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor
            -pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                ...
                {(unseenMessages[user._id] || 0)>0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center
